...
'''
# 臺北 cafe
@app.route('/cafe/taipei', methods=['GET'])
...
# 新竹 cafe
@app.route('/cafe/hsinchu', methods=['GET'])
...
# 高雄 cafe
@app.route('/cafe/kaohsiung', methods=['GET'])
...
'''
# 臺北市垃圾車點位路線資訊 (取得總筆數)
@app.route('/garbage_trucks/count', methods=['GET'])
def get_garbage_trucks_count_in_taipei():
...
# 臺北市垃圾車點位路線資訊 (分頁)
@app.route('/garbage_trucks/<int:page>', methods=['GET'])
def get_garbage_trucks_in_taipei_by_page(page):
    # 先對 垃圾車點位路線資訊 Web API 發送 GET 請求 (request)，取得對方伺服器的回應 (response)，
    # 其中回應帶有垃圾車點位路線資訊列表的資訊
    ...
    if 'page' in request.args:
        page = int(request.args['page']) # 取得 GET 請求的 page 值，並轉換成整數
...
# 臺北市垃圾車點位路線資訊 (全部)
@app.route('/garbage_trucks', methods=['GET'])
